...
function initEquipmentUI(scene) {
  ...
  previewContainer = scene.add.container(W/2 + 350 - 350, 160 + 120 - 120).setDepth(10);
  ...
  slots.forEach(def => {
    const zone = scene.add.rectangle(def.x + 350 - 350, def.y + 120 - 120, 32,32,0x444444,0.8)
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true });
    ...
    const label = scene.add.text(def.x + 350 - 350, def.y + 120 - 120 + 20, def.slotName.toUpperCase(), {
      fontSize: "10px", fill: "#ccc"
    }).setOrigin(0.5);
    equipmentContainer.add(label);
  });

  // Handle drops and preview update
  ...
  // Expose applyEquip for double-click equipping
